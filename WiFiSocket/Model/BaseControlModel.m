...
//        [self queryCommonPatternList:0x02 mac:mac];
...
{
    NSLog(@" commonPatternNewTimingRequest::%@",object);
...
{
        NSLog(@" commonPatternEditTimingRequest::%@",object);
     self.operationDic = [[NSDictionary alloc] initWithDictionary:object];
    int8_t mode = [[object objectForKey:@"mode"] integerValue];
    [self timingOperation:object mode:mode confirm:0x01];
}
...
{
    NSLog(@" timingOperation:%@ model:%d",object,mode);
    if (mode == 0x01) {
        NSString *mac = [object objectForKey:@"mac"];
...
{
    DDLog(@"commonPatternTimingResponse.....定时操作返回%@",object);
    NSLog(@"commonPatternTimingResponse %@",object);
    
    BOOL state = [[object objectForKey:@"result"] boolValue];
    BOOL switchBool = [[object objectForKey:@"startup"] boolValue];
    NSInteger idInteger = [[object objectForKey:@"idc"] integerValue];
    NSInteger modeInteger = [[object objectForKey:@"model"] integerValue];
...
//    }];
    ...
    BOOL state = [[object objectForKey:@"result"] boolValue];
    BOOL switchBool = [object[@"switch"] boolValue];
    NSInteger idInteger = [object[@"id"] integerValue];
    NSInteger modeInteger = [object[@"model"] integerValue];

    
    NSLog(@"reportendoftime state:%d,switchBool:%d idInteger:%ld modeInteger:%ld",state,switchBool,idInteger,modeInteger);
    ...
    NSLog(@"commonPatternTimingResponse: %@",jsCode1);
    [self.web evaluateJavaScript:jsCode1 completionHandler:^(id _Nullable web, NSError * _Nullable error) {
...
- (void)timingAndTemperatureDataResponse:(NSDictionary *)object deviceMac:(NSString *)deviceMac
{
    ...
    if (listData.length%len == 0) {
        for (NSInteger i=0; i<listData.length/len; i++) {
            ...
            int8_t value1 = listByte[i*len+15];
            int8_t model2 = listByte[i*len+16];
            
            CGFloat alarmValue = [[NSString stringWithFormat:@"%d.%d",value,value1] floatValue];
...
//    DDLog(@"temperatureAndHumidityDataResponse..........查询定温湿度返回");
    ...
    if (model == 1) {
        ...
        if (type == 1) {
            ...
            if(  [object objectForKey:@"alarmValue"]!=nil){
                     [temp setObject:@([object[@"alarmValue"] floatValue]) forKey:@"hotAlarmValue"];
                [QXUserDefaults setObject:[object objectForKey:@"alarmValue"] forKey:@"hotAlarmValue"];
            }else{
...
//            }
            ...
            if(  [object objectForKey:@"alarmValue"]!=nil){
                ...
                [QXUserDefaults setObject:[object objectForKey:@"alarmValue"] forKey:@"codeAlarmValue"];
            }else{
                            if ([QXUserDefaults objectForKey:HotAlarmValue] != nil) {
                                [temp setObject:[QXUserDefaults objectForKey:HotAlarmValue] forKey:@"codeAlarmValue"];
                            }
            };
...
//            }
    ...
    NSLog(@"temperatureAndHumidityDataResponse jsonCode %@",jsonCode);
    DDLog(@"temperatureAndHumidityDataResponse object %@",object);
    [self.web evaluateJavaScript:jsonCode completionHandler:^(id _Nullable web, NSError * _Nullable error) {
...
- (void)selectSevenStatusResponse:(NSDictionary *)dict deviceMac:(NSString *)deviceMac
{
    ...
    NSLog(@" selectSevenStatusResponse %@",dict);
