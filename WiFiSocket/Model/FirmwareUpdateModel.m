...
{
        DDLog(@"checkFirmwareVersionRequest : %@",object);
...
{
    DDLog(@"firmwareUpgradeRequest : %@",object);
    NSString *mac = [object objectForKey:@"mac"];
    uint8_t type = 0x01;
    uint8_t cmd = 0x09;
    uint8_t mode = 0x01;
...
{
      DDLog(@"FirmwareOperationResponse : %@",object);
    ...
    if (type == 2) {
        ...
        BOOL res = [[object objectForKey:@"isold"] boolValue];
        NSString *jsCode = [NSString stringWithFormat:@"checkFirmwareVersionResponse('%@',%d,'%@','%@')",deviceMac,res,new,current];
        DDLog(@"evaluateJavaScript 2: %@",jsCode);
        ...
        NSString *jsCode = [NSString stringWithFormat:@"firmwareUpgradeResponse('%@','%@',%d)",deviceMac,deviceMac,[object[@"result"] boolValue]];
          DDLog(@"evaluateJavaScript 1: %@",jsCode);
        [self.web evaluateJavaScript:jsCode completionHandler:^(id _Nullable web, NSError * _Nullable error) {
