...
//            DDLog(@"--------------发现设备：%@--------------发现包：%@",nameStr,property);
            ...
            int curint = ([self getByteWithoffset:useDataLoc+1] & 0xFF) <<8 |([self getByteWithoffset:useDataLoc+2]&0xFF);
            int newint =([self getByteWithoffset:useDataLoc+3] & 0xFF) <<8 |([self getByteWithoffset:useDataLoc+4]&0xFF);
            BOOL isold = newint>curint;
            if ([self.delegate respondsToSelector:@selector(FirmwareOperationResponse:deviceMac:)]) {
                [self.delegate FirmwareOperationResponse:@{@"result":@(result),@"type":@([self getByteWithoffset:useDataLoc]),
                                                           @"currentVersion":current,@"newVersion":new,@"isold":@(isold),
...
//                                                                         @"worknettime":@(worknettime)
        ...
        if (cmd == 0x02) {
            ...
            if ([self.delegate respondsToSelector:@selector(commonPatternTimingResponse:deviceMac:)]) {
                ...
                uint8_t model = [self getByteWithoffset:useDataLoc];
                BOOL startup = YES;
                uint8_t startupint =0x01;
                uint8_t idc = [self getByteWithoffset:useDataLoc+1];
                
                DDLog(@"定时回复 mode:%d idc:%d",model,idc);
                
                if(model==1){
                    //普通模式
                     startupint =  [self getByteWithoffset:useDataLoc+7] ;
                    
                }else if(model == 2){
                    // 高级模式
                  startupint =  [self getByteWithoffset:useDataLoc+12] ;
                }
                
                DDLog(@"定时回复 startupint:%d",startupint);
                     startup =( startupint== 0x01);
                
                [self.delegate commonPatternTimingResponse:@{@"result":@(result),
                                                             @"startup":@(startup),
                                                                @"model":@(model),
                                                                @"idc":@(idc)} deviceMac:deviceMac];
            }
        }else if(cmd == 0x08){
            //设置倒计时回复
...
//                }
            ...
            NSInteger mode = [self getByteWithoffset:useDataLoc+16];
            ...
            //查询7种状态
                    ...
                    NSLog(@" selectSevenStatusResponse in HandingDataModel 0x40 request");
            
            if ([self.delegate respondsToSelector:@selector(selectSevenStatusResponse:deviceMac:)]) {
                uint8_t status0 = [self getByteWithoffset:useDataLoc - 1];
...
//            CGFloat max_current = [[[NSString alloc] initWithFormat:@"%d",(([self getByteWithoffset:startLoc+12]<<8)+[self getByteWithoffset:startLoc+13])] floatValue];
            ...
            [self printReceiveData];
            useDataLoc = useDataLoc -1;
            NSInteger model = [self getByteWithoffset:useDataLoc];
            NSInteger ID = [self getByteWithoffset:useDataLoc + 1];
            BOOL sWitch;
            uint8_t startup;
            if (model == 1) {
                sWitch = (BOOL)[self getByteWithoffset:useDataLoc + 2];//on
              startup = [self getByteWithoffset:useDataLoc + 6];//startup
            }else{
                sWitch = (BOOL)[self getByteWithoffset:useDataLoc + 6];//on
          startup =  [self getByteWithoffset:useDataLoc + 11];
          
            }
            
            NSLog(@"receiveReportTiming startup:%d ",startup);
...
//                [self.delegate reportendoftime:@{@"result":@(result),@"model":@(model),@"id":@(ID),@"switch":@(sWitch)} deviceMac:deviceMac];
                // result::表示操作成功
                [self.delegate reportendoftime:@{@"result":@(YES),
                                                 @"model":@(model),
                                                 @"id":@(ID),
                                                 @"switch":@(result)} deviceMac:deviceMac];
            }
            ...
            if ([self.delegate respondsToSelector:@selector(temperatureSensorStateResponse:deviceMac:)]) {
                ...
                NSLog(@" selectSevenStatusResponse in HandingDataModel 0x13 report");
...
-(void)printReceiveData
{
    if(self.recevieData){
        Byte *testByte = (Byte*)[self.recevieData bytes];
        NSLog(@"printReceiveData");
        for (int i = 0;i<self.recevieData.length;i++) {
            NSLog(@"recevieData[%d]:%x",i,testByte[i]);
        }
    }
}
