...
- (void)deviceHistoryDataResponse:(NSDictionary *)object deviceMac:(NSString *)deviceMac
{
    ...
    if (res) {
        ...
        if (interval == 1 || interval == 3) {
            ...
            for (int i = 0; i < num; i++) {
                ...
                if (dataIneteger > num - 1) {
                    ...
                    NSLog(@"第%ld个数据 ----- %f",dataIneteger,e);
        ...
        NSString *startTime1 = [NSString stringWithFormat:@"%ld/%ld/%ld",year+2000,month,day];
        NSString *jsCode = [NSString stringWithFormat:@"deviceHistoryDataResponse('%@','%@',%ld,%ld,'%@')",deviceMac,startTime1,muArr.count,interval,json];
        [self.web evaluateJavaScript:jsCode completionHandler:^(id _Nullable web, NSError * _Nullable error) {
